...
public class OM{
  static void sum(int[] array, int custo){
    int count=0;
    for(int i=1; i<array.length; i++ ){
      if(array[i]==1001){System.out.println("impossivel");return;}
      count+=-1*array[i];
    }
    System.out.println("rendimento optimo: "+(count-array[0]*custo));
  }
  static void prim_alg(Grafo g, int custo){
    int[] pai = new int[ g.num_vertices() + 1 ];
    int[] dist = new int[ g.num_vertices() + 1 ];
    boolean[] ok = new boolean[ g.num_vertices() + 1 ];
    int v=1;
    Queue<Arco> fila = new LinkedList<Arco>();

    for(int i=0; i<dist.length; i++){ dist[i]=1001;}

    dist[v]=0;
    Heapmin heap = new Heapmin(dist,g.num_vertices());
    while(!heap.isEmpty()){
      v=heap.extractMin();
      if(g.find_arc(pai[v],v)!=null){
        fila.add(g.find_arc(pai[v],v));
      }

      ok[v]=true;
      for(Arco y: g.adjs_no(v)){
        int w= y.extremo_final();
        if(!ok[w] && (-1*g.find_arc(v,w).valor_arco()<dist[w])){
          dist[w]=-1*g.find_arc(v,w).valor_arco();
          pai[w]=v;
  ...
  public static void main(String[] args){
    ...
    for(int i=0; i<nlig; i++){
      ...
      int b=stdin.nextInt();
      int c=stdin.nextInt();
      g.insert_new_arc(a,b,c);
      g.insert_new_arc(b,a,c);
    }
    prim_alg(g,custo);
  }
}
